...
%e calcola la media delle potenze. Put it in the array V, same length as t

for j=1:DIMT(2)
  %tempi del vettore t

    %media dei valori compresi tra FROM e TO
     V(j)=db(mean(ps(FROM:TO,j)));
...
%find peaks
[m(2,:),m(1,:),~,p]=findpeaks(V,t,'MinPeakProminence',10,'MinPeakDistance', 0.15, 'MaxPeakWidth', 0.2, 'MinPeakHeight',-245,'Annotate','extents');

%plot graph with peaks
figure(k+2);
findpeaks(V,t,'MinPeakProminence',10,'MinPeakDistance', 0.15, 'MaxPeakWidth', 0.2, 'MinPeakHeight',-245,'Annotate','extents')
% plot(t,V,m(1,:),m(2,:),'o');
% xlim([t(1) t(end)]);
% grid on;
